...
int doMapsHide(bool isMainLib,std::vector<std::string>& so_list) {
    LOGI("call riru_hide,pid:%d",getpid());
...
//        LOGD("%" PRIxPTR"-%" PRIxPTR" %s %ld %s", (uintptr_t) maps_tmp->addr_start,
//             (uintptr_t) maps_tmp->addr_end, maps_tmp->perm, maps_tmp->offset,
//             maps_tmp->pathname);
        if (isMainLib) {
            // 主lib中，隐藏除开自己的其他so文件
            ...
            for(std::string so_name:so_list){
                if (!strstr(maps_tmp->pathname, so_name.c_str())) {
                    continue;
                }
            }
        }

        LOGI("find maps relate feature filed:%s", maps_tmp->pathname);
